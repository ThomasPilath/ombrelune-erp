...
function confirmAction(title: string, description: string): Promise<boolean> {
  const dialog = document.createElement("dialog"); dialog.className = "m-auto w-[min(32rem,calc(100%-2rem))] rounded-2xl border bg-surface p-0 text-ink shadow-2xl backdrop:bg-slate-950/70";
  ...
  return new Promise(resolve => { let settled = false; const finish = (value: boolean): void => { if (settled) return; settled = true; dialog.close(); dialog.remove(); resolve(value); }; dialog.querySelector("[data-cancel]")!.addEventListener("click", () => finish(false)); dialog.querySelector("[data-confirm]")!.addEventListener("click", () => finish(true)); dialog.addEventListener("cancel", event => { event.preventDefault(); finish(false); }, { once: true }); });
}
...
function renderBuyback(outlet: HTMLElement, articles: Record<string, unknown>[]): void {
  ...
  const draw = (preserveText = false): void => {
    ...
    const buybackRows = articles.map(article => { const values = stock(article); return { article, values, suggested: values.maximum == null ? "—" : Math.max(0, values.maximum - values.current), quantity: quantities.get(String(article.id)) ?? 0 }; });
    const mobileBuyback = `<div class="grid max-h-[32rem] gap-3 overflow-y-auto p-3 md:hidden">${buybackRows.map(({ article, values, suggested, quantity }) => `<section class="rounded-xl border bg-surface-muted p-4"><div class="flex items-start justify-between gap-3"><div><h3 class="font-bold">${escapeHtml(article.article)}</h3><p class="mt-1 text-sm text-muted">${money.format(Number(article.prix_achat))} PO · stock ${values.current}${values.maximum == null ? "" : `/${values.maximum}`}</p></div><strong>${money.format(quantity * Number(article.prix_achat))} PO</strong></div><div class="mt-4 grid grid-cols-2 gap-3"><p class="text-sm"><span class="block text-xs font-bold text-muted">Qté suggérée</span><strong class="text-green-600">${suggested}</strong></p><label class="text-xs font-bold text-muted">Qté rachetée<input data-buyback="${escapeHtml(article.id)}" type="number" min="0" value="${quantity}" class="mt-1 min-h-11 w-full rounded-xl border bg-surface px-3 text-base text-ink" aria-label="Quantité rachetée pour ${escapeHtml(article.article)}"></label></div></section>`).join("")}</div>`;
    const desktopBuyback = `<div class="hidden max-h-[32rem] overflow-auto md:block"><table class="w-full min-w-[46rem] text-left"><thead class="sticky top-0 bg-surface-muted text-sm"><tr><th class="px-4 py-3">Article</th><th class="px-4 py-3">Prix d’achat</th><th class="px-4 py-3">Stock</th><th class="px-4 py-3">Qté suggérée</th><th class="px-4 py-3">Qté rachetée</th><th class="px-4 py-3">Coût</th></tr></thead><tbody>${buybackRows.map(({ article, values, suggested, quantity }) => `<tr class="border-t"><td class="px-4 py-3 font-bold">${escapeHtml(article.article)}</td><td class="px-4 py-3">${money.format(Number(article.prix_achat))} PO</td><td class="px-4 py-3 text-muted">${values.current}${values.maximum == null ? "" : ` /${values.maximum}`}</td><td class="px-4 py-3 font-bold text-green-600">${suggested}</td><td class="px-4 py-3"><input data-buyback="${escapeHtml(article.id)}" type="number" min="0" value="${quantity}" class="min-h-11 w-24 rounded-xl border bg-surface px-3" aria-label="Quantité rachetée pour ${escapeHtml(article.article)}"></td><td class="px-4 py-3 font-bold">${money.format(quantity * Number(article.prix_achat))} PO</td></tr>`).join("")}</tbody></table></div>`;
    outlet.innerHTML = `${panel("Matières premières rachetées", `${mobileBuyback}${desktopBuyback}`)}
      <section class="mt-5 rounded-2xl border border-brand bg-surface p-6 text-center shadow-[var(--shadow-panel)]"><p class="text-sm font-bold text-muted">Total à payer</p><p class="mt-1 text-3xl font-black text-brand">${money.format(total())} PO</p></section>
      <section class="mt-5 rounded-2xl border bg-surface p-5 shadow-[var(--shadow-panel)]"><label for="buyback-summary" class="text-lg font-bold">Résumé pour Discord / RP</label><textarea id="buyback-summary" class="mt-3 min-h-48 w-full rounded-xl border bg-surface-muted p-4 font-mono text-sm">${escapeHtml(previousText ?? summary())}</textarea><div class="mt-3 grid gap-3 sm:grid-cols-3"><button id="apply-buyback-text" class="min-h-11 rounded-xl border font-bold">Appliquer le texte</button><button id="copy-buyback-text" class="min-h-11 rounded-xl border border-brand font-bold text-brand">Copier le texte</button><button id="submit-buyback" ${total() ? "" : "disabled"} class="min-h-11 rounded-xl bg-brand font-bold text-white disabled:opacity-40">Valider le paiement</button></div></section>`;
...
async function renderOrders(outlet: HTMLElement): Promise<void> {
  ...
  const confirmStatusChange = (title: string, description: string, confirmLabel: string): Promise<boolean> => {
    const dialog = document.createElement("dialog"); dialog.className = "m-auto w-[min(32rem,calc(100%-2rem))] rounded-2xl border bg-surface p-0 text-ink shadow-2xl backdrop:bg-slate-950/70";
    dialog.innerHTML = `<div class="border-b p-5"><h2 class="text-xl font-bold">${escapeHtml(title)}</h2><p class="mt-2 text-sm text-muted">${escapeHtml(description)}</p></div><div class="flex justify-end gap-3 p-4"><button type="button" data-cancel class="min-h-11 rounded-xl border px-4 font-bold">Annuler</button><button type="button" data-confirm class="min-h-11 rounded-xl bg-brand px-4 font-bold text-white">${escapeHtml(confirmLabel)}</button></div>`;
    document.body.append(dialog); dialog.showModal();
    return new Promise(resolve => { let settled = false; const finish = (value: boolean): void => { if (settled) return; settled = true; dialog.close(); dialog.remove(); resolve(value); }; dialog.querySelector("[data-cancel]")!.addEventListener("click", () => finish(false)); dialog.querySelector("[data-confirm]")!.addEventListener("click", () => finish(true)); dialog.addEventListener("cancel", event => { event.preventDefault(); finish(false); }, { once: true }); });
  };
  ...
  outlet.querySelectorAll<HTMLButtonElement>("[data-cancel-order]").forEach(button => button.addEventListener("click", () => {
    const dialog = document.createElement("dialog"); dialog.className = "m-auto w-[min(32rem,calc(100%-2rem))] rounded-2xl border bg-surface p-0 text-ink shadow-2xl backdrop:bg-slate-950/70";
    dialog.innerHTML = `<div class="border-b p-5"><h2 class="text-xl font-bold">Annuler la commande ?</h2><p class="mt-2 text-sm text-muted">La commande de <strong>${escapeHtml(button.dataset.cancelLabel)}</strong> restera dans l’historique avec le statut « Annulée ».</p></div><div class="flex justify-end gap-3 p-4"><button type="button" data-keep class="min-h-11 rounded-xl border px-4 font-bold">Conserver</button><button type="button" data-confirm-cancel class="min-h-11 rounded-xl bg-red-600 px-4 font-bold text-white">Confirmer l’annulation</button></div>`;
    document.body.append(dialog); dialog.showModal();
    const close = (): void => { dialog.close(); dialog.remove(); };
    dialog.querySelector("[data-keep]")!.addEventListener("click", close);
    dialog.addEventListener("cancel", event => { event.preventDefault(); close(); }, { once: true });
    dialog.querySelector("[data-confirm-cancel]")!.addEventListener("click", async () => { try { await callRpc("gerer_commande", { p_commande_id: button.dataset.cancelOrder, p_employe_id: sessionId(), p_action: "Annulation" }, z.null()); close(); showToast("Commande annulée et action journalisée.", "success"); await renderOrders(outlet); } catch (error) { showToast(error instanceof Error ? error.message : "Annulation impossible.", "error"); } });
  }));
  ...
  outlet.querySelectorAll<HTMLButtonElement>("[data-delete-order]").forEach(button => button.addEventListener("click", () => {
    const dialog = document.createElement("dialog"); dialog.className = "m-auto w-[min(32rem,calc(100%-2rem))] rounded-2xl border bg-surface p-0 text-ink shadow-2xl backdrop:bg-slate-950/70";
    dialog.innerHTML = `<div class="border-b p-5"><h2 class="text-xl font-bold">Supprimer définitivement ?</h2><p class="mt-2 text-sm text-muted">La commande de <strong>${escapeHtml(button.dataset.deleteLabel)}</strong> sera supprimée de la base. Une trace de cette suppression et de l’employé restera dans le journal backend.</p></div><div class="flex justify-end gap-3 p-4"><button type="button" data-keep class="min-h-11 rounded-xl border px-4 font-bold">Conserver</button><button type="button" data-confirm-delete class="min-h-11 rounded-xl bg-red-600 px-4 font-bold text-white">Supprimer définitivement</button></div>`;
    document.body.append(dialog); dialog.showModal(); const close = (): void => { dialog.close(); dialog.remove(); };
    dialog.querySelector("[data-keep]")!.addEventListener("click", close); dialog.addEventListener("cancel", event => { event.preventDefault(); close(); }, { once: true });
    dialog.querySelector("[data-confirm-delete]")!.addEventListener("click", async () => { try { await callRpc("gerer_commande", { p_commande_id: button.dataset.deleteOrder, p_employe_id: sessionId(), p_action: "Suppression" }, z.null()); close(); showToast("Commande supprimée définitivement.", "success"); await renderOrders(outlet); } catch (error) { showToast(error instanceof Error ? error.message : "Suppression impossible.", "error"); } });
  }));
}

async function renderPermits(outlet: HTMLElement): Promise<void> {
  const clients = (await rows("clients_actifs", "id,nom_prenom,hibou,client_permits(id,type,status)")).sort(alphabetically);
  ...
  const openEditor = (initialClient?: Record<string, unknown>, permit?: Record<string, unknown>): void => {
    ...
    const dialog = document.createElement("dialog"); dialog.className = "m-auto max-h-[calc(100%-2rem)] w-[min(38rem,calc(100%-2rem))] overflow-y-auto rounded-2xl border bg-surface p-0 text-ink shadow-2xl backdrop:bg-slate-950/70";
  ...
  const draw = (): void => {
    ...
    results.innerHTML = filtered.length ? `<div class="divide-y">${filtered.map(client => {
      ...
      return `<section class="p-4 sm:p-5"><div class="flex flex-wrap items-center justify-between gap-3"><div><strong class="text-lg">${escapeHtml(client.nom_prenom)}</strong><span class="mt-1 block text-sm text-muted">Hibou ${escapeHtml(client.hibou ?? "—")}</span></div><button data-create-permit="${escapeHtml(client.id)}" class="min-h-11 rounded-xl bg-brand px-4 font-bold text-white">Créer un dossier</button></div>${permits.length ? `<div class="mt-4 grid gap-2">${permits.map(permit => `<div class="flex min-h-12 items-center justify-between gap-3 rounded-xl border px-3"><span><strong>${escapeHtml(typeLabels[String(permit.type)] ?? permit.type)}</strong><span class="ml-2 text-sm text-muted">${escapeHtml(statusLabels[String(permit.status)] ?? permit.status)}</span></span><button data-edit-permit="${escapeHtml(permit.id)}" data-client-id="${escapeHtml(client.id)}" class="grid size-10 place-items-center rounded-lg border text-brand" aria-label="Modifier le permis de ${escapeHtml(client.nom_prenom)}" title="Modifier">${icon("edit", "size-5")}</button></div>`).join("")}</div>` : '<p class="mt-4 rounded-xl bg-surface-muted p-3 text-sm text-muted">Cette personne ne possède encore aucun dossier de permis.</p>'}</section>`;
    }).join("")}</div>` : '<p class="p-8 text-center text-muted">Aucune personne correspondante.</p>';
    results.querySelectorAll<HTMLButtonElement>("[data-create-permit]").forEach(button => button.addEventListener("click", () => openEditor(clients.find(client => String(client.id) === button.dataset.createPermit)!)));
...
async function renderDirection(outlet: HTMLElement): Promise<void> {
  ...
  const renderDomain = async (domainId: string): Promise<void> => {
    buttons.forEach(button => {
      ...
      button.classList.toggle("border-brand", selected);
