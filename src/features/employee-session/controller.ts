...
async function openActivityDialog(trigger: HTMLButtonElement): Promise<void> {
  ...
  dialog.className = "m-auto max-h-[calc(100dvh-2rem)] w-[min(64rem,calc(100%-2rem))] overflow-y-auto rounded-2xl border bg-canvas p-0 text-ink shadow-2xl backdrop:bg-backdrop";
  dialog.innerHTML = `<div class="sticky top-0 z-20 flex items-start justify-between gap-4 border-b bg-canvas/95 p-5 backdrop-blur-xl"><div><p class="text-sm font-bold uppercase tracking-[.14em] text-accent">Gestion</p><h2 class="mt-1 text-2xl font-black">Mon activité</h2><p class="mt-1 text-sm text-muted">Votre activité et vos performances pour la période en cours.</p></div><button type="button" data-close class="${secondaryIconButton}" aria-label="Fermer">×</button></div><div data-activity-content class="p-4 sm:p-6">${asyncState("loading")}</div><div class="sticky bottom-0 flex justify-end border-t bg-canvas/95 p-4 backdrop-blur-xl"><button type="button" data-close class="${primaryButton}">Fermer</button></div>`;
...
function requestDirectionPassword(employee: EmployeeRow): Promise<boolean> {
  ...
  document.body.insertAdjacentHTML("beforeend", `<dialog id="direction-password-dialog" class="m-auto w-[min(28rem,calc(100%-2rem))] rounded-2xl border bg-surface p-0 text-ink shadow-2xl backdrop:bg-backdrop">
    <form class="p-6">
      <p class="text-sm font-bold text-accent">Accès Direction</p>
      ...
      <input id="direction-password" name="password" type="password" required autocomplete="current-password" class="${field} min-h-12">
      <p data-password-error class="mt-3 text-sm font-semibold text-danger" role="alert" hidden>Mot de passe incorrect.</p>
      <div class="mt-5 grid grid-cols-2 gap-3"><button type="button" data-cancel class="${secondaryButton}">Annuler</button><button type="submit" class="${primaryButton}">Déverrouiller</button></div>
    </form>
  </dialog>`);
...
function renderSidebarSelector(employees: EmployeeRow[], session: EmployeeSession | null): void {
  ...
  container.innerHTML = `<div class="group relative w-full">
      <button id="open-employee-activity" type="button" class="mb-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border bg-surface px-3 text-sm font-bold text-accent hover:bg-surface-muted disabled:opacity-40" aria-label="Ouvrir mes statistiques" ${session ? "" : "disabled"}>${icon("chart", "size-5 shrink-0")}<span>Mes Statistiques</span></button>
      ...
      <p id="active-employee-role" class="mt-2 truncate px-3 text-sm text-muted"><span class="font-bold">Rôle :</span> <span class="font-semibold text-ink">${escapeHtml(activeEmployee?.grade ?? "Aucun employé")}</span></p>
      <span id="employee-session-help" role="tooltip" class="pointer-events-none absolute left-0 top-[calc(100%+.45rem)] z-50 w-full translate-y-1 rounded-lg bg-backdrop px-3 py-2 text-xs text-white opacity-0 shadow-xl transition group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100">L’employé choisi est sauvegardé pendant 12 heures.</span>
    </div>`;
...
function createSelectionDialog(employees: EmployeeRow[], onSelected: () => void): HTMLDialogElement {
  ...
  document.body.insertAdjacentHTML("beforeend", `<dialog id="employee-session-dialog" class="m-auto w-[min(32rem,calc(100%-2rem))] rounded-2xl border bg-surface p-0 text-ink shadow-2xl backdrop:bg-backdrop">
    <form id="employee-session-form" class="p-6 sm:p-7">
      <span class="mb-5 grid size-12 place-items-center rounded-2xl bg-brand/10 text-accent">${icon("badge", "size-6")}</span>
      <p class="text-sm font-bold text-accent">Bienvenue sur Ombrelune</p>
      ...
      <button type="submit" class="mt-5 inline-flex min-h-12 w-full items-center justify-center rounded-xl border border-accent bg-brand px-4 py-2 font-bold text-white hover:bg-brand-strong">Continuer</button>
    </form>
  </dialog>`);
...
function renderLoadError(): void {
  const container = document.querySelector<HTMLElement>("#employee-session-control");
  if (!container) return;
  container.innerHTML = `<p class="text-sm font-bold text-danger">Employés indisponibles</p><button id="retry-employees" type="button" class="mt-2 min-h-11 w-full rounded-xl border px-3 py-2 text-sm font-semibold hover:bg-surface-muted">Réessayer</button>`;
