...
export function renderApp(pageId: string): () => void {
  ...
  document.querySelector<HTMLDivElement>("#app")!.innerHTML = `<div id="app-shell" class="app-shell lg:grid lg:grid-cols-[16rem_minmax(0,1fr)]">
    <div id="sidebar-backdrop" class="sidebar-backdrop fixed inset-0 z-30 bg-backdrop lg:hidden" aria-hidden="true"></div>
    <aside id="sidebar" class="app-sidebar fixed inset-y-0 left-0 z-40 flex w-[16rem] flex-col border-r bg-surface p-4 lg:sticky lg:top-0 lg:h-screen" aria-label="Navigation principale" aria-hidden="true">
      <div class="mb-6 px-2"><div class="flex min-h-11 items-center"><div class="flex min-w-0 items-center gap-2"><span class="grid size-10 shrink-0 place-items-center rounded-xl bg-brand text-white shadow-md" aria-hidden="true">${icon("moon", "size-8")}</span><strong class="sidebar-collapsible truncate font-[Cinzel] text-2xl leading-none text-ink">Ombrelune</strong></div><button id="collapse-sidebar" class="ml-auto hidden size-11 shrink-0 place-items-center rounded-xl border lg:grid xl:hidden" aria-label="Réduire le menu" aria-expanded="true">${icon("arrow")}</button><button id="close-menu" class="ml-auto grid size-11 shrink-0 place-items-center rounded-xl hover:bg-surface-muted lg:hidden" aria-label="Fermer le menu">${icon("close")}</button></div></div>
    ...
    <div id="app-main-column" class="min-w-0">
      <header class="sticky top-0 z-20 grid min-h-16 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1 border-b bg-canvas/90 px-4 py-2 backdrop-blur-xl sm:px-7 md:grid-cols-[minmax(12rem,1fr)_minmax(0,2fr)_minmax(12rem,1fr)]">
        <div class="flex min-w-0 items-center gap-3"><button id="open-menu" class="grid size-11 shrink-0 place-items-center rounded-xl border bg-surface lg:hidden" aria-label="Ouvrir le menu" aria-controls="sidebar" aria-expanded="false">${icon("menu")}</button>
          <div class="min-w-0"><p class="truncate text-xs font-bold uppercase tracking-[.14em] text-accent">${page.eyebrow}</p><h1 class="truncate font-semibold">${page.title}</h1></div>
        </div>
        <p class="col-span-1 col-start-2 hidden text-center text-sm leading-tight text-muted md:block">${page.description}</p>
        <span class="col-start-3 row-start-1" aria-hidden="true"></span>
      </header>
