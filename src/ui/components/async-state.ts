...
export function asyncState(kind: StateKind, detail?: string, retryLabel?: string): string {
  ...
  return `<div class="grid min-h-56 place-items-center p-8 text-center" role="${kind === "error" ? "alert" : "status"}" aria-live="polite">
    <div><span class="mx-auto mb-4 grid size-12 place-items-center rounded-2xl bg-surface-muted text-accent">${icon(state.iconName, "size-6")}</span>
    <h3 class="font-bold">${state.title}</h3><p class="mt-1 max-w-md text-sm text-muted">${escapeHtml(detail ?? state.description)}</p>${kind === "error" && retryLabel ? `<button type="button" data-retry class="mt-4 min-h-11 rounded-xl border border-accent px-4 font-bold text-accent">${escapeHtml(retryLabel)}</button>` : ""}</div>
  </div>`;
}
